...
bool	iarduino_I2C_Matrix_8x8::begin			(void){																	//	Параметр: отсутствует
		...
		//	Если адрес не указан, то ищим модуль на шине I2C:															//
			if(valAddrTemp==0){																							//
				for(int i=1; i<127; i++){																				//	Проходим по всем адресам на шине I2C
					if( checkAddress(i)										){									//	Если на шине I2C есть устройство с адресом i, то ...
					if( readByte(i,REG_MODEL)          == DEF_MODEL_8X8		){									//	Если у модуля с адресом i в регистре «MODEL»   хранится значение DEF_MODEL_8X8     (светодиодная матрица ), то ...
					j = readByte(i,REG_CHIP_ID); if( j == DEF_CHIP_ID_FLASH	||	j==DEF_CHIP_ID_METRO	){		//	Если у модуля с адресом i в регистре «CHIP_ID» хранится значение DEF_CHIP_ID_FLASH (идентификатор модулей Flash), или DEF_CHIP_ID_METRO (идентификатор модулей Metro), то ...
                    uint8_t tmp = readByte(i, REG_ADDRESS);
					if( tmp>>1     == i					){	valAddrTemp=i; i=128;	}else	//	Если у модуля с адресом i в регистре «ADDRESS» хранится значение i                 ( адрес + младший бит ), то cчитаем что модуль обнаружен, сохраняем значение i как найденный адрес и выходим из цикла.
					if( tmp        == 0xFF				){	valAddrTemp=i; i=128;	}		//	Если у модуля с адресом i в регистре «ADDRESS» хранится значение 0xFF              ( адрес не задавался  ), то cчитаем что модуль обнаружен, сохраняем значение i как найденный адрес и выходим из цикла.
					}																									//
					}																									//
					}																									//
				}																										//
		...
		//	Проверяем наличие модуля и значения его регистров:																	//
			if( checkAddress(valAddrTemp)            == false										){return false;}	//	Если на шине I2C нет устройств с адресом valAddrTemp,                     то возвращаем ошибку инициализации
        //  W: 04, R: 08
			if( readByte(valAddrTemp,REG_MODEL)      != DEF_MODEL_8X8								){return false;}	//	Если значение регистра «MODEL»   не совпадает со значением DEF_MODEL_8X8, то возвращаем ошибку инициализации
        //  W: 07, R: 3C
			j = readByte(valAddrTemp,REG_CHIP_ID); if( j!=DEF_CHIP_ID_FLASH && j!=DEF_CHIP_ID_METRO	){return false;}	//	Если значение регистра «CHIP_ID» не совпадает со значением DEF_CHIP_ID_FLASH и DEF_CHIP_ID_METRO, то возвращаем ошибку инициализации
        //  W: 06, R: 12 (12>>1=9)
            uint8_t tmp = readByte(valAddrTemp,REG_ADDRESS); 
			if( tmp >>1 != valAddrTemp									){					//	Если значение регистра «ADDRESS» не совпадает с адресом модуля,           то ...
			if( tmp     != 0xFF										){return false;}}	//	Если значение регистра «ADDRESS» не совпадает со значением 0xFF,          то возвращаем ошибку инициализации
		//	Определяем значения переменных:																				//	
			valAddr=valAddrTemp;																						//	Сохраняем адрес модуля на шине I2C.
			valVers=readByte(valAddr,REG_VERSION);																//	Сохраняем версию прошивки модуля.
			        readByte(valAddr,REG_FLAGS_0);																//	Читаем флаги регистра «REG_FLAGS_0» в никуда.
...
void	iarduino_I2C_Matrix_8x8::drawImage		(uint8_t* arr, uint8_t i, uint8_t j){									//	Параметры:				массив с изображением, [функция анимации появления] , и или [тип памяти массива].
		...
		//	Загружаем изображение:																						//
			for(int k=0; k<8; k++){ if(memArr==X8_IMG_RAM){ data[k]=arr[k]; }else{ }}	//	Получаем данные для массива data по адресу arr из ОЗУ или ПЗУ.
...
void iarduino_I2C_Matrix_8x8::changeBus(std::string bus)
{
