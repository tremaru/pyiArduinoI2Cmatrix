...
bool	iarduino_I2C_Matrix_8x8::getPullI2C		(void){																	//	Параметр:				отсутствует.
			if(valAddr){																								//	Если модуль был инициализирован, то ...
...
bool	iarduino_I2C_Matrix_8x8::setPullI2C		(bool f){																//	Параметр:				флаг подтяжки линий шины I2C.
			if(valAddr){																								//	Если модуль был инициализирован, то ...
			//	Считываем первые два регистра: регистр флагов и регистр битов:											//
				if(_readBytes(REG_FLAGS_0,2)==false ){ return false; }													//	Читаем 2 байта начиная с регистра «REG_FLAGS_0» в массив «data».
			//	Проверяем поддерживает ли модуль управление подтяжкой линий шины I2C:									//
...
void	iarduino_I2C_Matrix_8x8::drawImage		(uint8_t* arr, uint8_t i, uint8_t j){									//	Параметры:				массив с изображением, [функция анимации появления] , и или [тип памяти массива].
		...
		//	Загружаем изображение:																						//
			for(int k=0; k<8; k++){ if(memArr==X8_IMG_RAM){ data[k]=arr[k]; }}	//	Получаем данные для массива data по адресу arr из ОЗУ или ПЗУ.
