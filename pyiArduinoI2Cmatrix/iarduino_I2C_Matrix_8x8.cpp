...
bool	iarduino_I2C_Matrix_8x8::begin			(void){																	//	Параметр: отсутствует
		...
		//	Если адрес не указан, то ищим модуль на шине I2C:															//
			if(valAddrTemp==0){																							//
				for(int i=1; i<127; i++){																				//	Проходим по всем адресам на шине I2C
					...
					if( objI2C->readByte(i,REG_MODEL)          == DEF_MODEL_8X8		){									//	Если у модуля с адресом i в регистре «MODEL»   хранится значение DEF_MODEL_8X8     (светодиодная матрица ), то ...
                    ...
                    uint8_t tmp = objI2C->readByte(i, REG_ADDRESS);
        ...
        //  W: 04, R: 08
			if( objI2C->readByte(valAddrTemp,REG_MODEL)      != DEF_MODEL_8X8								){return false;}	//	Если значение регистра «MODEL»   не совпадает со значением DEF_MODEL_8X8, то возвращаем ошибку инициализации
        //  W: 07, R: 3C
        ...
        //  W: 06, R: 12 (12>>1=9)
            uint8_t tmp = objI2C->readByte(valAddrTemp,REG_ADDRESS); 
